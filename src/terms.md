# RideSheet Terms of Service

**Service Provider:** Full Path LLC

**Last Updated:** December 31, 2025

## 1. Acceptance of Terms

By using RideSheet, you agree to these Terms of Service between you and Full Path LLC ("we," "us," "our"). If you do not agree, do not install or use RideSheet.

**Who these terms apply to:**

- Individual users who install RideSheet
- Organizations that deploy RideSheet
- Anyone who modifies or distributes RideSheet

## 2. Description of Service

RideSheet is a free, open-source transportation management tool provided by Full Path LLC that operates as a Google Apps Script within Google Sheets. No data is transmitted to or hosted on servers controlled by the RideSheet developers. RideSheet helps you:

- Manage customer and trip information
- Create driver manifests
- Calculate trip distances and times
- Organize transportation schedules
- Generate high-quality operations data for reporting and analysis

**RideSheet is provided "AS IS" without any warranties or guarantees.**

## 3. Open Source License

RideSheet is open-source software. All code is released under the [MIT license](https://github.com/full-path/ridesheet/blob/main/LICENSE).

**You are free to:**

- Use RideSheet for any purpose (personal or commercial)
- Modify the code to suit your needs
- Distribute original or modified versions
- Review and audit the source code

**Requirements:**

- Include appropriate license notices when distributing
- Acknowledge that modified versions are not official RideSheet releases. Your modified version must be governed by your own terms of service.
- Comply with the terms of the open-source license

## 4. Google Account Requirements

### Account Access

- RideSheet requires a Google account to function
- You must authorize RideSheet to access specific Google services
- You can revoke access at any time on a per-user basis at [https://myaccount.google.com/connections](https://myaccount.google.com/connections)

### Your Responsibilities

- Maintain the security of your Google account
- Keep your account credentials confidential
- Be responsible for all activities under your account
- Comply with Google's Terms of Service

## 5. Acceptable Use

### You May Use RideSheet To:

- Manage transportation operations
- Organize trip and customer data
- Generate driver manifests
- Calculate routes and distances
- Any other lawful transportation management purpose

### You May NOT Use RideSheet To:

- Violate any applicable laws or regulations
- Infringe on others' intellectual property rights
- Harass, abuse, or harm individuals
- Distribute malware or malicious code
- Attempt to gain unauthorized access to systems
- Violate Google's Terms of Service or Acceptable Use Policy

## 6. Your Data and Privacy

### Data Ownership

- You own all data you enter into RideSheet
- RideSheet does not claim any rights to your data
- Your data stays in your Google account

### Data Responsibility

You are responsible for:

- The accuracy of data you enter
- Compliance with privacy laws (GDPR, CCPA, etc.)
- Obtaining necessary consents from individuals whose data you store and process
- Protecting sensitive information appropriately
- Creating backups of important data

For maximum data control and security, we **strongly** recommend you only store sensitive data in organizational Google Workspace accounts.

### Privacy Policy

Our [Privacy Policy](../privacy) explains how RideSheet accesses and uses your data. By using RideSheet, you also agree to our Privacy Policy.

## 7. No Warranties

**RideSheet is provided "AS IS" and "AS AVAILABLE" without warranties of any kind, either express or implied, including but not limited to:**

- Warranties of merchantability
- Fitness for a particular purpose
- Non-infringement
- Accuracy or reliability
- Uninterrupted or error-free operation
- Security or data protection

**We do not warrant that:**

- RideSheet will meet your specific requirements
- RideSheet will be available at all times
- All errors will be corrected
- RideSheet is free from viruses or harmful components
- Results obtained will be accurate or reliable

## 8. Limitation of Liability

**To the maximum extent permitted by law:**

### No Liability for Damages

We are not liable for any indirect, incidental, special, consequential, or punitive damages, including but not limited to:

- Loss of profits or revenue
- Loss of data
- Loss of business opportunities
- Service interruptions
- Costs of substitute services

### Circumstances Include

Even if such damages arise from:

- Use or inability to use RideSheet
- Errors or bugs in the software
- Unauthorized access to your data
- Third-party actions
- Any other cause related to RideSheet

### Maximum Liability

If we are found liable despite these limitations, our total liability shall not exceed $100 USD or the amount you paid to use RideSheet (which is $0 for the free version).

## 9. Indemnification

You agree to indemnify and hold harmless Full Path LLC, its members, employees, contractors, and RideSheet project contributors from any claims, damages, losses, liabilities, and expenses (including legal fees) arising from:

- Your use or misuse of RideSheet
- Your violation of these Terms
- Your violation of any laws or regulations
- Your violation of third-party rights
- Data you enter or manage in RideSheet

## 10. Third-Party Services

### Google Services

RideSheet integrates with Google services (Sheets, Drive, Docs, Maps). Your use of these services is governed by:

- Google's Terms of Service
- Google's Privacy Policy
- Google Workspace Acceptable Use Policy

**We are not responsible for:**

- Changes to Google's services
- Google service outages
- Google's terms or policies
- Google's data practices

### Google Maps

Address validation and distance calculations use Google Maps API, subject to:

- Google Maps Platform Terms of Service
- Google Maps API usage limits
- Google's privacy policies

## 11. Modifications to Service

### We May Modify RideSheet

- Add, change, or remove features
- Update requirements or dependencies
- Change how RideSheet operates
- Discontinue RideSheet entirely

**We are not obligated to:**

- Provide updates or support
- Maintain backward compatibility
- Notify you of changes
- Continue developing RideSheet

### Updates Are Optional

- RideSheet does not auto-update
- You choose when to update
- You can continue using older versions
- You can fork and maintain your own version

## 12. Modifications to Terms

### We May Update These Terms

We reserve the right to modify these Terms at any time.

**When we update:**

- The "Last Updated" date will change
- Material changes will be noted in release notes
- Continued use after changes constitutes acceptance

**Your Options:**

- Review terms periodically
- Stop using RideSheet if you disagree with changes
- Fork the project and use your own version

## 13. Support and Maintenance

### No Guaranteed Support

- RideSheet is provided without support obligations
- We may provide support at our discretion
- Community support may be available
- Response times are not guaranteed

### Where to Get Help

- GitHub Issues: [https://github.com/full-path/ridesheet/issues](https://github.com/full-path/ridesheet/issues)
- Community forums (if available)
- [Documentation and guides](https://docs.ridesheet.org)
- Self-help through open-source code review

## 14. Termination

### You May Terminate

You can stop using RideSheet at any time by:

- Revoking permissions on a per-user basis at [https://myaccount.google.com/connections](https://github.com/full-path/ridesheet/issues)
- Deleting your RideSheet spreadsheet
- Uninstalling the Apps Script

### We May Terminate

We may discontinue RideSheet or restrict access if:

- You violate these Terms
- Required by law
- Necessary to protect our rights
- We decide to discontinue the project

### Effect of Termination

- Your data remains in your Google account
- You lose access to RideSheet's features
- These Terms survive termination where applicable

## 15. Dispute Resolution

### Informal Resolution

Before filing any formal dispute, please contact Full Path LLC to attempt informal resolution.

### Governing Law

These Terms are governed by the laws of the State of Oregon, without regard to conflict of law provisions.

### Jurisdiction

Any disputes shall be resolved in the courts of Multnomah County, Oregon.

### Limitation Period

Any claim must be filed within one year of the claim arising.

## 16. General Provisions

### Entire Agreement

These Terms, along with the Privacy Policy, constitute the entire agreement between you and RideSheet.

### Severability

If any provision is found unenforceable, the remaining provisions remain in effect.

### No Waiver

Failure to enforce any provision does not waive our right to enforce it later.

### Assignment

You may not assign these Terms. We may assign these Terms to any successor.

### No Partnership

These Terms do not create any partnership, agency, or employment relationship.

### Force Majeure

We are not liable for failures caused by circumstances beyond our reasonable control.

## 17. Specific Use Cases

### Commercial Use

You may use RideSheet for commercial transportation operations. All terms still apply. We **strongly** recommend you only store sensitive information in organizational Google Workspace accounts.

### Government/Public Sector Use

Government entities may use RideSheet subject to these Terms and applicable procurement regulations.

### Healthcare Transportation (NEMT)

**Important:** RideSheet is NOT inherently HIPAA-compliant out of the box. The developers of RideSheet are not 'Business Associates' as defined by HIPAA, as we never have access to your data. Users are solely responsible for ensuring their Google Workspace environment is configured for HIPAA compliance and for executing a BAA with Google. Do not use RideSheet to store Protected Health Information (PHI) unless:

- You have proper Business Associate Agreements
- You have configured Google Workspace for HIPAA compliance
- You accept full responsibility for HIPAA compliance

### Educational Use

Educational institutions may use RideSheet for transportation management with all terms applying.

## 18. Open Source Contributions

### Contributing Code

If you contribute code to RideSheet:

- You grant a license to use your contributions
- You represent that you have the right to grant such license
- Contributions are subject to the project's open-source license
- You agree to the project's contributor guidelines

### Reporting Issues

When reporting bugs or issues:

- Do not include sensitive personal data
- Provide sufficient detail for reproduction
- Be respectful and constructive
- Understand that fixes are not guaranteed

## 19. Export Compliance

You agree to comply with all applicable export and import laws and regulations. You will not export, re-export, or transfer RideSheet to prohibited countries or individuals.

## 20. Contact Information

**For questions about these Terms:**

Full Path LLC
- Email: info@ridesheet.org
- GitHub: [https://github.com/full-path/ridesheet](https://github.com/full-path/ridesheet)
- Response time: Best effort, no guarantee

---

## Summary in Plain Language

**What you're agreeing to:**

- RideSheet is free and open-source
- Use it at your own risk (no warranties)
- We're not liable if something goes wrong
- Your data stays in your Google account
- You own your data and are directly responsible for how you share it
- Follow the law and Google's rules
- You can modify and share RideSheet
- We can change or discontinue RideSheet
- Disputes are governed by Oregon law

**What we're promising:**

- To keep RideSheet open-source
- To respect your privacy (see [Privacy Policy](../privacy))
- To not claim ownership of your data

**What we're NOT promising:**

- That RideSheet will work perfectly
- That it will always be available
- That we'll fix every bug
- That we'll provide support
- That we won't make changes

---

**By using RideSheet, you acknowledge that you have read, understood, and agree to be bound by these Terms of Service.**
